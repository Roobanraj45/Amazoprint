...
import { z } from 'zod';
import { createFreelancerDirectSellingProduct } from '@/app/actions/direct-selling-actions';
...
export default function NewFreelancerProductPage() {
    ...
    const [attributes, setAttributes] = useState<any[]>([
        {
            ...
            type: 'color',
            options: [
                { id: 'opt-1', name: 'Black', value: '#000000', priceAdjustment: 0, stock: 50, isActive: true },
                { id: 'opt-2', name: 'White', value: '#FFFFFF', priceAdjustment: 0, stock: 50, isActive: true }
            ]
        },
        {
            id: 'attr-2',
            ...
            type: 'button',
            options: [
                { id: 'opt-3', name: 'S', priceAdjustment: 0, stock: 25, isActive: true },
                { id: 'opt-4', name: 'M', priceAdjustment: 0, stock: 50, isActive: true },
                { id: 'opt-5', name: 'L', priceAdjustment: 20, stock: 25, isActive: true }
            ]
        }
    ]);

    // Price Slabs State
    const [priceSlabs, setPriceSlabs] = useState<any[]>([
        { id: 'slab-1', quantity: 10, price: 900, isActive: true },
        { id: 'slab-2', quantity: 25, price: 2100, isActive: true },
        { id: 'slab-3', quantity: 50, price: 4000, isActive: true }
    ]);

    // GST Tax Slabs State
    const [taxSlabs, setTaxSlabs] = useState<any[]>([
        { id: 'tax-1', name: 'GST (18%)', rate: 18, type: 'percentage', isInclusive: true, isActive: true }
    ]);
    ...
    const [specs, setSpecs] = useState({
        material: 'Premium Grade 100% Bio-Washed Material',
        finish: 'Matte Finish / HD Inks',
        brand: 'Designer Merch',
        originCountry: 'India',
        minOrderQuantity: 1,
        warranty: '100% Quality Guaranteed',
        careInstructions: 'Machine wash cold, do not iron directly on print',
    });
    ...
    const { register, handleSubmit, setValue, watch, control, formState: { errors } } = useForm<FormData>({
        ...
        defaultValues: {
            ...
            stockQuantity: 100,
            minStockLevel: 5,
            sku: `DS-${Date.now().toString().slice(-6)}`,
            isFeatured: true,
            isActive: true,
            textAllowed: false,
            offerBadge: 'NEW LAUNCH',
            tags: 'merch, custom, print, premium',
        }
    });
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            ...
            await createFreelancerDirectSellingProduct(payload);
            toast({ title: 'Product Created Successfully!', description: 'Your product has been submitted for admin approval.' });
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-black text-slate-900 dark:text-white flex items-center gap-2">
                                <Store size={20} className="text-amber-500" /> List Direct Selling Product
                            </h1>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
                ...
                <Card className="rounded-3xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                    <CardHeader>
                        ...
                        <CardDescription className="text-xs">Basic details for the storefront catalog</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                ...
                                <Input 
                                    {...register('name')} 
                                    placeholder="e.g. Vintage Anime Oversized Cotton T-Shirt"
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Input {...register('slug')} placeholder="vintage-anime-oversized-t-shirt" className="rounded-xl font-mono text-xs" />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="space-y-1.5">
                                ...
                                <Input {...register('category')} placeholder="e.g. Apparel & Merch" className="rounded-xl" />
                            </div>

                            <div className="space-y-1.5">
                                <Label className="text-xs font-bold uppercase tracking-wider">Master SKU</Label>
                                <Input {...register('sku')} placeholder="e.g. MERCH-TSHIRT-01" className="rounded-xl font-mono text-xs" />
                            </div>

                            <div className="space-y-1.5">
                                <Label className="text-xs font-bold uppercase tracking-wider">HSN / SAC Code</Label>
                                <Input {...register('hsnCode')} placeholder="e.g. 61091000" className="rounded-xl font-mono text-xs" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Textarea 
                                ...
                                rows={4} 
                                placeholder="Describe product material, print technique, fit, style and packaging..."
                                className="rounded-xl"
                            />
                        </div>
                    </CardContent>
                ...
                <Card className="rounded-3xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                    <CardHeader>
                        ...
                        <CardDescription className="text-xs">Upload high-resolution mockup and live product photos</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                ...
                <Card className="rounded-3xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                    <CardHeader>
                        ...
                        <CardDescription className="text-xs">Manage commercial selling rate, MRP discount, and overall inventory</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Input type="number" step="0.01" {...register('costPrice')} placeholder="Strikethrough MRP" className="rounded-xl" />
                            </div>
                ...
                {/* 4. WooCommerce Style Attributes & Options */}
                <Card className="rounded-3xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between">
                        <div>
                            ...
                            <CardDescription className="text-xs">Amazon / Myntra style swatches and size buttons with live stock tracking</CardDescription>
                        </div>
                        <Button
                            type="button"
                            size="sm"
                ...
                <Card className="rounded-3xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between">
                        <div>
                            ...
                            <CardDescription className="text-xs">Amazon Business / B2B bulk packages (e.g. 10 Pcs, 25 Pcs, 50 Pcs)</CardDescription>
                        </div>
                        <Button
                            type="button"
                            size="sm"
