...
import { getMessagesForUser, sendUserMessage, markUserMessagesAsRead } from '@/app/actions/user-message-actions';
...
export default function FreelancerMessagesPage() {
  ...
  const fetchMessages = async (showLoader = false) => {
    ...
    try {
      const data = await getMessagesForUser();
      ...
      if (unreadAdminMessages.length > 0) {
        ...
        await markUserMessagesAsRead(adminSender, 'admin');
      }
    } catch (error) {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    startSendingTransition(async () => {
      try {
        await sendUserMessage(msg, att || undefined);
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-5xl mx-auto bg-slate-50 dark:bg-zinc-950 rounded-3xl border border-slate-200/60 dark:border-zinc-800/80 overflow-hidden shadow-xl">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white dark:bg-zinc-900 border-b border-slate-100 dark:border-zinc-800/60 z-10 shrink-0">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-2xl bg-violet-500/10 text-violet-600 flex items-center justify-center border border-violet-500/20">
            <ShieldAlert className="h-5 w-5" />
          </div>
          <div>
            <h2 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-wider">Creator Support Desk</h2>
            <p className="text-[10px] text-slate-400 font-bold">Direct line to Amazoprint Creator & Contest Support Administrators</p>
          </div>
        </div>
        <Button 
          variant="outline" 
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 min-h-0 bg-slate-50/50 dark:bg-zinc-950/20">
        {isLoading && messages.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-6 text-slate-400 dark:text-zinc-500 max-w-sm mx-auto">
            ...
            <p className="text-[11px] text-slate-400 mt-1">Submit tickets or chat with support regarding design templates, contest payouts, or verification.</p>
          </div>
        ) : (
          messages.map((m) => {
              ...
              <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} w-full`}>
                <div className={`flex items-start gap-2.5 max-w-[75%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                  <Avatar className="h-8 w-8 shrink-0 border border-slate-200 dark:border-zinc-800 shadow-sm">
                    <AvatarFallback className={isMe ? "bg-slate-900 text-white font-bold text-xs" : "bg-violet-600 text-white font-bold text-xs"}>
                      {isMe ? 'CR' : 'AD'}
                    </AvatarFallback>
                  </Avatar>
      ...
      <form onSubmit={handleSendMessage} className="p-4 bg-white dark:bg-zinc-900 border-t border-slate-100 dark:border-zinc-800/60 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className={`h-10 w-10 rounded-xl shrink-0 transition-colors ${showAttachmentInput ? 'bg-violet-55 text-violet-600 dark:bg-violet-950/20' : 'border-slate-200'}`}
          ...
          <Input 
            ...
            disabled={isSending}
            className="flex-1 bg-slate-50 dark:bg-zinc-950 border-slate-200 focus-visible:ring-violet-500/20 focus-visible:border-violet-500/50 rounded-xl h-10 text-xs"
          />

          <Button 
            type="submit" 
            disabled={isSending || (!inputText.trim() && !attachmentUrl.trim())}
            className="h-10 px-4 rounded-xl shrink-0 bg-violet-600 hover:bg-violet-700 text-white font-extrabold gap-1.5 text-xs shadow-md shadow-violet-600/20"
          >
            {isSending ? (
