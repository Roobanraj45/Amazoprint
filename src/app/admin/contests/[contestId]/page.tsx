import { getAdminContestDetails } from "@/app/actions/contest-actions";
import { notFound } from "next/navigation";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { format } from "date-fns";
import { SubmissionPreview } from "@/app/client/contests/[contestId]/submission-preview";
import { getSession } from "@/lib/auth";
import { Trophy } from "lucide-react";


export default async function AdminContestDetailsPage({ params }: { params: { contestId: string }}) {
    ...
    const contest = await getAdminContestDetails(contestId);
    
    if (!contest) {
        notFound();
    }

    const submittedEntries = contest.participants.filter(p => p.submission);
                ...
                <CardHeader>
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <CardDescription>
                                Created by {contest.user.name} ({contest.productName} - {contest.subProductName})
                            </CardDescription>
