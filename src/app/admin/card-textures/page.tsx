...
import { z } from 'zod';
import { getCardTextures, createCardTexture, updateCardTexture, deleteCardTexture } from '@/app/actions/card-texture-actions';
...
import { Loader2, PlusCircle, Edit, Trash2, ImageIcon, Library, Upload, Sparkles, CheckCircle2, Layers, FolderOpen, FileImage, Check } from 'lucide-react';
...
export default function CardTexturesPage() {
  const [cardTextures, setCardTextures] = useState<CardTexture[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isFormOpen, setFormOpen] = useState(false);
  const [editingCardTexture, setEditingCardTexture] = useState<CardTexture | null>(null);
  const { toast } = useToast();

  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await getCardTextures();
      setCardTextures(data);
    } catch (error) {
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to load card texture specifications.' });
    } finally {
      setIsLoading(false);
    }
  }, [toast]);
  ...
  const handleFormSubmit = async (data: z.infer<typeof cardTextureSchema>) => {
    try {
      if (editingCardTexture) {
        await updateCardTexture(editingCardTexture.id, data);
        toast({ title: 'Success', description: 'Card texture updated.' });
      } else {
        await createCardTexture(data);
        toast({ title: 'Success', description: 'Card texture created.' });
      }
      setFormOpen(false);
      setEditingCardTexture(null);
      await loadData();
    } catch (error: any) {
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteCardTexture(id);
      toast({ title: "Success", description: "Card texture deleted." });
      await loadData();
    } catch (error: any) {
      toast({ variant: "destructive", title: "Error", description: error.message });
    }
    ...
    <div className="space-y-8 max-w-[1600px] mx-auto pb-12">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <Badge variant="outline" className="bg-indigo-500/20 text-indigo-300 border-indigo-500/30 text-xs px-3 py-1 rounded-full font-semibold backdrop-blur-md">
                ...
                Premium Texture Engine
              </Badge>
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white">Card Texture Management</h1>
            <p className="text-slate-300 text-sm sm:text-base mt-2 max-w-2xl">
              Create, organize, and configure premium tactile paper textures, preview imagery, and pricing add-on rules for specialty textured cards.
            </p>
          </div>
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <Dialog open={isFormOpen} onOpenChange={(open) => {
              setFormOpen(open);
              if (!open) setEditingCardTexture(null);
            }}>
              <DialogTrigger asChild>
                <Button size="lg" className="h-12 rounded-2xl font-extrabold bg-indigo-600 hover:bg-indigo-700 text-white shadow-xl shadow-indigo-500/20 px-6 transition-all hover:scale-[1.02] w-full sm:w-auto">
                  <PlusCircle className="mr-2 h-5 w-5" /> Add Card Texture
                </Button>
              </DialogTrigger>
              <CardTextureForm
                onSubmit={handleFormSubmit}
                cardTexture={editingCardTexture}
      ...
      {isLoading ? (
          <div className="flex flex-col justify-center items-center h-[40vh] gap-4">
            ...
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Loading card texture specifications...</p>
          </div>
        ) : cardTextures.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
            {cardTextures.map((ct) => (
                <Card key={ct.id} className="group border border-slate-200/80 dark:border-slate-800/80 rounded-3xl overflow-hidden bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col">
                    <div className="aspect-square relative bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800 p-6 group-hover:bg-indigo-50/20 dark:group-hover:bg-indigo-950/20 transition-colors flex items-center justify-center overflow-hidden">
                        {ct.imageUrl ? (
                            <Image src={resolveImagePath(ct.imageUrl)} alt={ct.name} fill className="object-cover p-2 filter drop-shadow-md group-hover:scale-105 transition-transform duration-500 rounded-2xl" />
                        ) : (
                            <div className="flex flex-col items-center justify-center h-full gap-2">
                                <ImageIcon className="h-10 w-10 text-slate-300 dark:text-slate-700" />
                                <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-600">No Texture Image</span>
                            </div>
                        )}
                        <Badge variant={ct.isActive ? 'default' : 'secondary'} className={`absolute top-3 right-3 h-5 text-[9px] font-extrabold px-2 rounded-full shadow-sm ${ct.isActive ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                            {ct.isActive ? 'Active' : 'Inactive'}
                        </Badge>
                    </div>
                    <CardHeader className="p-5 flex-1 flex flex-col justify-between space-y-3">
                        <div>
                            <CardTitle className="text-lg font-extrabold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-1">{ct.name}</CardTitle>
                            <CardDescription className="text-xs font-semibold text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">Slug: {ct.slug}</CardDescription>
                        </div>
                    ...
                    <CardFooter className="p-3 bg-slate-50/50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-1 flex-shrink-0">
                        <Button variant="ghost" size="sm" className="h-8 px-2.5 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 dark:hover:bg-slate-800 dark:hover:text-indigo-400 font-bold text-xs transition-colors" onClick={() => { setEditingCardTexture(ct); setFormOpen(true); }}>
                        ...
                        <AlertDialog>
                            ...
                            <AlertDialogHeader>
                                <AlertDialogTitle className="text-xl font-bold">Delete Card Texture?</AlertDialogTitle>
                                <AlertDialogDescription className="text-sm font-medium">This action will permanently delete this card texture specification and its associated pricing rules. This cannot be undone.</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel className="h-10 rounded-xl font-bold border-slate-200 dark:border-slate-800">Cancel</AlertDialogCancel>
                                <AlertDialogAction className="h-10 rounded-xl font-bold bg-destructive hover:bg-destructive/90 text-white shadow-lg shadow-destructive/20" onClick={() => handleDelete(ct.id)}>Permanently Delete</AlertDialogAction>
                            </AlertDialogFooter>
                            </AlertDialogContent>
                        </AlertDialog>
                    </CardFooter>
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm text-center px-4 gap-4">
              ...
              <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">No Card Textures Configured</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 max-w-md">There are currently no custom card textures available in the system. Click &apos;Add Card Texture&apos; above to create your first texture.</p>
          </div>
        )}
    </div>
  );
...
function CardTextureForm({ onSubmit, cardTexture, onClose }: {
  onSubmit: (data: z.infer<typeof cardTextureSchema>) => void;
  cardTexture: CardTexture | null;
  onClose: () => void;
}) {
  const [isImageBrowserOpen, setImageBrowserOpen] = useState(false);
  const [currentFolder, setCurrentFolder] = useState('card-textures');

  const { register, handleSubmit, formState: { errors, isSubmitting }, reset, control, setValue, watch } = useForm<z.infer<typeof cardTextureSchema>>({
    resolver: zodResolver(cardTextureSchema),
    defaultValues: {
      name: '',
      slug: '',
      imageUrl: '',
  ...
  useEffect(() => {
    if (cardTexture) {
      reset({
        name: cardTexture.name,
        slug: cardTexture.slug,
        imageUrl: cardTexture.imageUrl || '',
        description: cardTexture.description || '',
        amount: Number(cardTexture.amount || 0),
        isActive: cardTexture.isActive ?? true,
      });
    } else {
      reset({
        name: '',
      ...
      <DialogHeader className="p-8 pb-8 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white relative overflow-hidden border-b border-slate-800 flex-shrink-0">
        ...
        <div className="flex items-center justify-between relative z-10">
            <div>
                ...
                <DialogTitle className="text-2xl font-bold tracking-tight text-white">{cardTexture ? 'Edit Card Texture' : 'Create Card Texture'}</DialogTitle>
                <DialogDescription className="text-slate-300 text-sm mt-1">Configure texture display names, URL slugs, pricing surcharges, and preview imagery.</DialogDescription>
            </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex-1 flex flex-col min-h-0 overflow-hidden">
        <div className="p-8 space-y-6 bg-slate-50/50 dark:bg-slate-950/50 overflow-y-auto flex-1">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Texture Display Name</Label>
              <Input id="name" {...register('name')} placeholder="e.g. Linen, Laid, Felt" className="h-10 rounded-xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus-visible:ring-indigo-500 font-semibold" />
            ...
            <div className="space-y-2">
              ...
              <Input id="slug" {...register('slug')} placeholder="e.g. linen-texture" className="h-10 rounded-xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus-visible:ring-indigo-500 font-semibold" />
          ...
          <div className="space-y-3">
            <Label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Texture Preview Image (PNG/JPG)</Label>
            <div className="flex gap-3">
                <Input {...register('imageUrl')} placeholder="/uploads/card-textures/texture.png" className="h-10 rounded-xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus-visible:ring-indigo-500 font-semibold flex-1" />
            ...
            {imageUrl ? (
                <div className="mt-4 relative h-40 w-40 rounded-2xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-inner mx-auto p-4 flex items-center justify-center">
                    <Image src={resolveImagePath(imageUrl)} alt="Card Texture Preview" fill className="object-cover p-2 filter drop-shadow-md rounded-2xl" />
                </div>
            ) : (
          ...
          <div className="space-y-2">
            <Label htmlFor="description" className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Texture Description</Label>
            <Textarea 
              id="description" 
              {...register('description')} 
              placeholder="Provide details about the paper texture, tactile feel, recommended print applications, or compatibility..."
          ...
          <div className="flex items-center justify-between p-5 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm">
            <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground font-medium">Allow customers to select this card texture during customization.</p>
            </div>
            <Controller
              name="isActive"
              control={control}
        ...
        <DialogFooter className="p-6 pt-4 border-t border-slate-200 dark:border-slate-800 bg-slate-100/50 dark:bg-slate-900/50 backdrop-blur-md flex items-center justify-end gap-3 flex-shrink-0">
          ...
          <Button type="submit" disabled={isSubmitting} className="h-10 rounded-xl font-bold bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/20 px-6">
            ...
            {cardTexture ? 'Update' : 'Create'} Card Texture
          </Button>
        </DialogFooter>
      </form>
    </DialogContent>
                 ...
                 <div className="max-w-xl mx-auto space-y-6 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            placeholder="e.g. card-textures, shapes, custom-templates"
                            value={folder}
