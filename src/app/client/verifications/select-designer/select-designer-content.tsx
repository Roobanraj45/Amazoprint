...
export default function SelectDesignerContent() {
  ...
  const handleSelectDesigner = (designerId: string) => {
    ...
    toast({ title: 'Designer Selected', description: 'Redirecting back to finalize verification request.' });
    router.push(`/client/verifications?${params.toString()}`);
  };

  const handleCancelSelection = () => {
    router.push(`/client/verifications?${searchParams.toString()}`);
  };
            ...
            <div className="space-y-1.5">
                <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
                    <button onClick={handleCancelSelection} className="hover:text-primary transition-colors flex items-center gap-1 group">
                        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" /> Back to Verification Form
                    </button>
                </div>
                <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                    Select Elite Reviewer
                </h1>
                <p className="text-xs text-muted-foreground font-semibold">
                    Browse our directory of top freelancers, inspect portfolio designs, and hire a design verifier.
                </p>
            </div>
                ...
                {selectedFreelancerForDetail ? (
                  <div className="space-y-6 flex-1 flex flex-col justify-between h-full">
                    ...
                    <div className="pt-4 border-t border-border mt-auto bg-card relative z-10">
                      ...
                      >
                        <UserCheck className="w-4.5 h-4.5" /> Select & Assign to Verification Request
                      </Button>
                    </div>
                  </div>
                ) : (
