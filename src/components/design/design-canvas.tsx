      ...
      >
        ...
        >
          ...
          {livePath && livePath.length > 0 && (
            <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', overflow: 'visible', pointerEvents: 'none', zIndex: 999 }}>
              <path d={generatePathD(livePath, false, safetyMargin, safetyMargin)} fill="none" stroke="hsl(var(--primary))" strokeWidth={1.5 / zoom} strokeDasharray={`${4/zoom} ${2/zoom}`} />
              {livePath.map((p, i) => (
                <React.Fragment key={i}>
                  {/* Control point lines */}
                  <line x1={p.cp1x + safetyMargin} y1={p.cp1y + safetyMargin} x2={p.x + safetyMargin} y2={p.y + safetyMargin} stroke="hsl(var(--muted-foreground))" strokeWidth={1 / zoom} />
                  <line x1={p.x + safetyMargin} y1={p.y + safetyMargin} x2={p.cp2x + safetyMargin} y2={p.cp2y + safetyMargin} stroke="hsl(var(--muted-foreground))" strokeWidth={1 / zoom} />
                  
                  {/* Anchor Point */}
                  <rect x={p.x + safetyMargin - 4 / zoom} y={p.y + safetyMargin - 4 / zoom} width={8 / zoom} height={8 / zoom} fill="white" stroke="hsl(var(--primary))" strokeWidth={1.5 / zoom} style={{ cursor: 'move' }}/>
                  
                  {/* Control Points */}
                  <circle cx={p.cp1x + safetyMargin} cy={p.cp1y + safetyMargin} r={3 / zoom} fill="hsl(var(--primary))" stroke="white" strokeWidth={0.5/zoom} style={{ cursor: 'pointer' }}/>
                  <circle cx={p.cp2x + safetyMargin} cy={p.cp2y + safetyMargin} r={3 / zoom} fill="hsl(var(--primary))" stroke="white" strokeWidth={0.5/zoom} style={{ cursor: 'pointer' }} />
                </React.Fragment>
              ))}
            </svg>
          )}
